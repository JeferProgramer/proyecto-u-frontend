...
function RegisterForm() {
    ...
    const afterSubmit = async () => {
        if (signupForm.license && signupForm.dni && signupForm.specialities && signupForm.education && Object.keys(formErrors).length === 0 && !userClientBtn) {
            ...
            formdata.append("dni" , signupForm.dni)
            formdata.append("license" , signupForm.license)
            formdata.append("education" , signupForm.education)
            
            const response = await axios.post(`${baseURL}/userpsychologist`, formdata)
